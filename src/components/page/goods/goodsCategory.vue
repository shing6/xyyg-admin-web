<template>
  <div class="container">
    ...
    <div class="handle-box">
      <el-input v-model="select_word" clearable class="filter-item" style="width: 200px;" placeholder="请输入分类名称"/>
    ...
    <el-table v-loading="listLoading" :data="tableData" @selection-change="selsChange" size="small" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" min-width="100" label="名称" prop="categoryName"/>
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        this.$axios.post('/apis/category/getCategoryByPage', params
        ).then((response) => {
      ...
      create() {
        ...
        this.$refs['addCategoryForm'].validate((valid) => {
          if (valid) {
            ...
            params.append('token', token);
            params.append('userId', userId);
            params.append('categoryName', this.addCategoryForm.content);
            this.$axios.post('/apis/category/insertCategory', params
            ).then((response) => {
              if (response.status === 200) {
      ...
      handleUpdate(row){
        ...
        this.editCategoryForm.content=row.categoryName;
      ...
      edit() {
        ...
        this.$refs['editCategoryForm'].validate((valid) => {
          if (valid) {
            ...
            params.append('categoryName', this.editCategoryForm.content);
            this.$axios.post('/apis/category/updateCategory', params
            ).then((response) => {
              if (response.status === 200) {
      ...
      hdelete(){
       ...
       this.$axios.post('/apis/category/deleteCategory', params
        ).then((response) => {
          if (response.status === 200) {
